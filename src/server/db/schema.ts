 ...
 */
export const mysqlTable = mysqlTableCreator((name) => `quiz-app_${name}`);
...
export const quizzes = mysqlTable(
  "quizzes",
  {
    ...
    token: varchar("token", { length: 14 }).notNull(),
    iconColor: varchar("icon_color", { length: 10 }),
...
export const questions = mysqlTable(
  "questions",
  {
    ...
    correctOption: varchar("correct_option", { length: 256 }).notNull(),
    createdAt: timestamp("created_at")
      .default(sql`CURRENT_TIMESTAMP`)
      .notNull(),
    updatedAt: timestamp("updatedAt").onUpdateNow(),
